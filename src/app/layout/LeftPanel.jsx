...
export default function App() {
  ...
  const handleMainCanvas = () => {
    setActivePhaseIndex(null);
    navigate("/project/id");
  };

  // rendering 3d plane for phases

  const renderNewPlane = (id) => {
    setActivePhaseIndex(id);

    navigate(`/project/id/phase/${id + 1}`);
  };
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          ...
          >
            {/* TOP BAR (Unchanged) */}
            <div
              className={`relative flex items-center justify-between px-4 py-4 min-h-[70px]`}
            >
            ...
            >
              ...
              <div className="flex-1 p-4 overflow-y-auto scrollbar-hide">
                ...
                <motion.button
                  ...
                  onClick={() => handleMainCanvas()}
                  className={`flex justify-between items-center text-sm text-white  px-2 py-2 w-full rounded-md mb-3 transition ${
                ...
                <div className="flex justify-between items-center text-gray-300 text-xs uppercase mt-4 mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <ChevronDown
                        size={14}
                        className={`${showPhases ? "rotate-0" : "-rotate-90"}`}
                      />
                    </motion.button>
                    ...
                    >
                      {phases.map((phase, idx) => (
                        <motion.div
                          ...
                          onClick={() => renderNewPlane(idx)}
                          // 2. Add cursor-pointer for better UX
                          className={`flex items-center justify-between w-full px-3 py-2 rounded-md text-sm cursor-pointer ${
                            // 3. Check against the activeIndex state
                ...
                <AnimatePresence>
                  {item.submenu && activeSubmenu === item.label && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: -10 }}
                      transition={{ duration: 0.2 }}
                      className="absolute left-[190px] top-0 w-[160px] bg-[#2C2C2C] border border-[#3E3E3E] rounded-lg shadow py-2"
                    >
          ...
          >
            <div className="bg-[#2C2C2C] p-5 rounded-xl border border-gray-700 w-[280px] text-center shadow-lg">
              ...
              <div className="flex justify-center gap-3 mt-2">
                <button
                  onClick={() => {
                    setPhases((p) => p.filter((_, i) => i !== phaseToDelete));
