...
export default function VerifyEmail() {
  ...
  const email = params.get("email");
  const clerkStatus =
    params.get("clerk_status") || params.get("__clerk_status");

  // 1. Redirect logic
  useEffect(() => {
    // If Clerk has finished loading and the user is signed in...
    if (isLoaded && isSignedIn) {
      // Optional: Check if the URL explicitly says they are verified
      if (clerkStatus === "verified") {
        console.log("Email successfully verified!");
      }
      // Redirect to your project page
      navigate("/", { replace: true });
    }
  }, [isLoaded, isSignedIn, navigate, clerkStatus]);
